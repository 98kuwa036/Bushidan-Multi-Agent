...
- **👁️ 検校 (Kengyo)**: ビジュアル・デバッガー - Kimi Vision + Playwright MCP
- **🧠 軍師 PDCA Engine**: Plan→Do(Kimi並列)→Check(256K+検校)→Act
...
- **🏯 5層+傭兵+検校**: 将軍→軍師→家老→大将(+傭兵)→足軽、検校がCheck補佐
...
- [システム概要](#システム概要)
- [検校 (Kengyo) ビジュアル・デバッガー](#検校-kengyo-ビジュアルデバッガー)
...
### v10.1 の目玉機能: PDCA Check フェーズのビジュアル検証

テキストベースの Check (Gunshi 256K) は論理的整合性を検証できますが、UI のレンダリング結果は「見ないとわからない」。**検校 (Kengyo)** はこの盲点を **Kimi K2.5 Vision + Playwright MCP** で埋めます。

```
PDCA Check フェーズ:

  ┌─────────────────────────────────┐
  │ 🔍 テキスト検証 (Gunshi 256K)   │  ← cross-file 整合性・コード品質
  │    重み: 80%                     │
  └─────────────┬───────────────────┘
                │
  ┌─────────────▼───────────────────┐
  │ 👁️ ビジュアル検証 (検校 Kengyo)  │  ← UI/UX レイアウト・表示品質
  │    重み: 20%                     │
  │                                  │
  │  Playwright MCP → Screenshot     │
  │  Kimi K2.5 Vision → Analysis     │
...
| **画像** | アイコン・画像の表示状態（欠落・歪み） |
| **レスポンシブ** | Desktop (1280px) / Tablet (768px) / Mobile (375px) |
| **リグレッション** | Before/After 比較による意図しない変更検出 |

### 動作条件

- **必須**: Kimi K2.5 クライアント (Vision 対応)
- **推奨**: Playwright MCP (Smithery) - スクリーンショット自動取得
- **自動判定**: タスクに UI キーワード (CSS, レイアウト, フロントエンド等) + URL が含まれる場合に起動
- **グレースフルデグラデーション**: 条件未達時はスキップ（テキスト検証のみ）
...
| **検校** | UI状態、ビューポート情報 | ビジュアル品質保証 | スクリーンショット解析計画 |

BDI により、実装者が Kimi でも Qwen3 でも「正確実装」という意図が一貫管理される。
検校は PDCA Check フェーズでビジュアル面からも品質を保証する。
...
**🏯 傭兵 + 検校 + PDCA Engine + BDI形式推論で挑む、鉄壁5層ハイブリッド 🏯**
